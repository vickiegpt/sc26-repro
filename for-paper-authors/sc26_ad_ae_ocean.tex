...
MPI CXL shim, and the experiment and plotting scripts for Figs.~3--5.
...
dependencies, compiles the OCEAN server, and downloads OCEAN's guest kernel
and image.  The experiment wrappers consume the downloaded resource paths;
reviewers do not need to edit launcher scripts manually.  Because setup and
experiment scripts install packages, create bridge/TAP interfaces, and launch
KVM guests, they should run only on a dedicated evaluation host.
...
KVM, root privileges, Tigon, GROMACS, or physical CXL hardware.  Install the
Python requirements, then replot all supplied Figures~3--5 data:
\begin{verbatim}
...
Use a dedicated Linux/KVM server and complete
\texttt{artifacts/setup-tools.sh}.  The script initializes OCEAN and SimCXL
and their dependencies, builds OCEAN's QEMU fork and SimCXL/gem5, downloads
the required kernels and disk images, and places those resources where the
experiment wrappers expect them.  Figure~3 requires one OCEAN VM and separate
SimCXL simulations.  Figure~4 uses up to six OCEAN VMs.  Figure~5 uses two
OCEAN VMs with guest-to-guest networking and MPI.
...
creates the OCEAN guests, copies the MPI STREAM workload from
\texttt{artifacts/tools/OCEAN/workloads/stream} to the VMs, constructs an MPI
hostfile containing the active nodes, and runs STREAM through
...
Copy \texttt{config.example.toml} to a new configuration file.  Figures~6 and
7 require a working Tigon checkout and its database image.  Figure~8 requires
...
\texttt{cxl\_switch\_lock\_bench\_mpi} benchmark.  Record the host, CPU,
...
and map size in \texttt{figures\_6\_9.toml}.  Collection is fail-closed:
\texttt{acknowledge\_dax\_writes} must be set to true and the configured
command must contain \texttt{--acknowledge-dax-write}.  The supplied example
limits the writable mapping to a page-aligned 2~MiB range.  After verifying
that range is safe, run:
\begin{verbatim}
python3 Ocean/script/reproduce_figures_6_9.py \
...
A successful run produces \texttt{fig9\_samples.csv},
\texttt{fig9\_params.csv}, and \texttt{fig9\_contention.csv}, plus Fig.~9a--c
in PDF and PNG and a checksum manifest.  The expected qualitative result is
